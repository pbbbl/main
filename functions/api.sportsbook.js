...
module.exports = (api) => {
  api.get("/sb/games", async (_, res) => {
    return await db
      .collection("games")
      .where("scheduleId", ">", `${DateTime.now().toISODate()}`)
      .limit(100)
      .get()
      .then((qsnapshot) => {
        try {
  ...
  api.get("/sb/games/:id", async (req, res) => {
    try {
...
function sendNotFound(res) {
  return res.status(404).send({
    ...
    status: 404,
    errorCode: "x-not-found",
  });
}
function sendError(res, error) {
  console.error("error", error);
  return res.status(500).send({
    error: `Something went wrong on our end. We're looking into it now!`,
...
function sendUnauthorized(res) {
  return res
    .status(401)
    .send({ error: "Unauthorized", status: 401, errorCode: "x-unauthorized" });
}
function sendSuccess(res, result) {
  return res.status(200).send(result);
}

// api.get("/sportspage", async (req, res) => {
//   const { ids } = req.query;
//   const games = await getGamesById(ids);
//   res.send(games);
// });
// function sportspageMiddleware(req, res, next) {
...
//     const options = {
//       method: "GET",
//       url: "https://api.sportspage.io/v1/sports",
//       headers: {
...
//     }
//   } catch (error) {
//     console.log("error getting sportspage data at useApi", {
//       error,
//       options,
//       eid,
//     });
