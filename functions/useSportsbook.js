const sportsbookCronJobModule = require("./useSportsbook.cronJob.js");
const sportsbookApiModule = require("./useSportsbook.api.js");
const sportsbookDbModule = require("./useSportsbook.db.js");
const modules = {
  ...(sportsbookApiModule.modules || {}),
  ...(sportsbookDbModule.modules || {}),
  ...(sportsbookCronJobModule.modules || {}),
};
module.exports = {
  useSportsbook: () => {
    return {
      modules,
      sportsbookCronJobModule,
      sportsbookApiModule,
      sportsbookDbModule,
    };
  },
...
//   return serialize({ TOSTRING }, { circular: true, ignoreFunction: true });
// };
// const IS = require("is");
...
//     status: "final",
//   });
//   return res.status(200).send({ future, live, past });
// });
...
//         return null;
//     } else {
//         const games = await Promise.all(ids.map((id)=>{
//             const ts = TS();
//              const item = await endpoints.gameById({id, cid});
//              const scheduleId = DateTime.fromISO(item.schedule.date).toISODate();
//              const cid = 'games';
//              const dbResult = await transformAndSaveGame({item, scheduleId, cid, ts});
//              return dbResult;
...
//         }),
//       );
//       return output && output?.length > 0 ? output.length === 1 ? output[0] : output : false;
//     } else {
//       return {
//         results: false,
//       };
